...
- 收藏弹窗前置自动化当前只优先覆盖 B 站视频详情页原生弹窗。
- 成功路径或失败路径都会显示更明显的提示：系统通知 + 页内红色/绿色 Toast + 扩展图标状态徽标。
- 大模型在由于未命中现有收藏夹而尝试**新建分类**时，会优先读取 `config/follow-categories.json` 中的 B 站默认标准分类库进行命名整理，避免杂乱。
